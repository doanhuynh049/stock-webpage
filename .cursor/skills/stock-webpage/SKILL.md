...
**Tabs**: Portfolio | **AI Analyst** | **AI Screening** | **AI News** | **AI Prediction** | Sector | ETF | VN30 | VN100 | Avg Down | **Exit Strategy** | Scoring rules | Principles
...
- **3-step pipeline**, each step a separate module:
  1. **Hard filter** (`src/lib/analysis/ai-screening.ts` → `screenUniverse`) — rejects ROE < 15%, Debt/Equity > 2.0, or liquidity (`price × 20d avg volume`) below threshold. FCF's 2-year-negative *rejection* still can never run (no multi-year FCF series is stored). The scoring slot is no longer dead, though — see step 2.
  2. **Weighted score** (same file) — each surviving metric min-max normalized 0–100, combined via user-configurable weights (default `0.25 roe + 0.20 revenueCagr + 0.20 epsGrowth3y + 0.15 debtToEquity(inv) + 0.10 fcf + 0.10 peg(inv)`, re-normalized to sum to 1). Top ~20 by score kept. **The FCF sub-score now uses `p_cash_flow_ratio`** (inverted — lower is better) when stored; when no surviving candidate has it, that 0.10 weight is *redistributed proportionally* across the other five (`redistributeFcfWeight`) rather than scoring everyone a flat 50, which used to compress the best-vs-worst spread by the FCF weight for zero informational gain. `epsGrowth3y` now reads the real `eps_growth_3y` column when present, falling back to the YoY proxy.
...
### AI News tab (Aug 2026) — per-ticker news classification + social sentiment, never a trading signal

Implements "AI News Reading & Sentiment Analysis": reads/classifies real news + Stocktwits social chatter for one ticker. AI's edge here is reading volume across sources, not predicting price — every report carries `disclaimer` inline (not a one-time UI banner), and no blended directional "score" is ever computed.

- **Pipeline**, 3 separate modules under `src/lib/analysis/`:
  1. **Ingestion** — reuses `getNewsLive(symbol)` (`news-service.ts`: Yahoo + Google News RSS **+ CafeF/VnExpress since Sep 2026**). CafeF/VnExpress publish general finance feeds, not per-ticker ones, so `fetchSymbolNews` fetches them whole and filters by a case-sensitive standalone-token match (`mentionsSymbol`) — lowercasing first would make 3-letter codes collide with ordinary Vietnamese prose. Before this, the only VN-official sources never reached per-ticker news, so `inferTrustTier` could almost never return `vn_official` for a VN company. No direct Reuters/Bloomberg/SEC EDGAR API (paid, or N/A — VN companies don't file with EDGAR); `inferTrustTier()` (`news-classification.ts`) infers "tier1_global" from Google News' publisher attribution when it surfaces a Reuters/Bloomberg/AP-sourced article, "vn_official" from a CafeF/VnExpress source string.
  2. **News classification** (`news-classification.ts` → `classifyNewsItems`) — one LLM call classifies all fetched items into `category` (partnership/earnings/regulatory/management/macro/analyst_rating/other) / `sentiment` / `time_horizon` / `confidence` / `reasoning`. `headline`/`source`/`timestamp`/`link` are always copied from the real `NewsItem` — the model is never asked for them, so it structurally can't fabricate a source. Rule-based keyword fallback (bilingual VN/EN patterns) when the LLM is unavailable, confidence pinned to 0.35 to signal lower certainty.
  3. **Social sentiment** (`social-sentiment.ts` + `src/lib/providers/stocktwits.ts`) — Stocktwits public stream only (free, no key). Bot/spam filtered by exact-duplicate-body dedup, a per-user post cap, and a newborn-account-age heuristic. `buzz_change_pct` is an approximation vs. whatever history the endpoint actually returns (disclosed via `methodology_note`) — **not** a true trailing-7-day baseline, since the public API doesn't expose that range without auth. Returns `null` (not zero) on total fetch failure — a `0` `post_volume` only means "checked, found nothing." **Since Sep 2026 a ticker found by `lookupIndexStock` (i.e. known VN30/VN100-listed) skips the network call entirely** and returns a "Not checked" snapshot: Stocktwits carries no Vietnamese-exchange listings, so the only possible outcomes were an empty stream or a same-named foreign security that the provider's `region !== "VN"` guard then discarded. Spending one request per holding to reach a foregone conclusion was pure latency on the Portfolio Overview path.
- **Aggregation** (`news-sentiment.ts` → `buildNewsSentimentReport`): merges the two into `NewsSentimentReport`. `conflicts[]` explicitly flags when news tone and social tone disagree, or when a buzz spike is large (≥100%) — reported neutrally, since a spike can be a catalyst or a scandal. Tier-1/VN-official news outweighs social sentiment in the trust-weighted summary, never via a single blended number.
- **Route**: `POST /api/analysis/news-sentiment` (session auth) — `{ symbol }`, loads per-user provider keys like `/api/analyst`/`/api/analysis/ai-screen`.
- **UI**: `NewsSentimentPanel` (`src/components/analysis/news-sentiment-panel.tsx`) — ticker input (same pattern as `StockEvaluationPanel`), conflicts banner, news list with trust/category/sentiment/horizon/confidence badges, social sentiment card with sample-size text always paired with any %. Last ticker+result persisted to `vnstocks:news-sentiment-state`; per-ticker report cached `vnstocks:news-sentiment-{SYM}` (**2h TTL** — news/sentiment refreshes over hours, deliberately longer than AI Analyst/Screening's 30-min TTL).
- **Portfolio Overview (Aug 2026)** — `news-sentiment-portfolio.ts` → `buildPortfolioNewsOverview`: runs the **full** LLM-backed pipeline for your top `MAX_OVERVIEW_HOLDINGS` (10) holdings by weight, in parallel — an explicit user choice over the cheaper rule-based-only option, since unlike AI Analyst (1 LLM call total via `skipLlm`) or AI Screening (1 call for up to 20 candidates), this is genuinely N separate LLM calls, one per holding, capped at 10 to bound worst-case cost/rate-limit exposure on large portfolios. Skipped holdings are listed, never silently dropped (`skippedSymbols`). `overallNewsStance`/`overallSocialStance` (exported `newsStance`/`socialStance` from `news-sentiment.ts`) are computed server-side and attached per row — the client only ever reads plain string fields, never imports these as values (see the client/server split note in `ai-screening-pattern.mdc`). Route: `GET /api/analysis/news-sentiment/portfolio`. Auto-runs on tab open (cached `vnstocks:news-sentiment-portfolio`, 2h) with a table of expandable rows reusing the same detail view as the ticker lookup below it. **This one IS background-prefetched** (unlike the ad-hoc ticker lookup, which isn't — no sensible default ticker to warm there).

- **Lazy + background loading (extended Aug 2026)**: every lazily-loaded tab/sub-tab loads on open AND is background-prefetched once after first paint (idle callback), so tab switches are instant — VN30/VN100/ETF (`/api/analysis/bundle`), **AI Analyst** (`/api/analyst/portfolio`, 1 LLM call), **AI Screening** (`/api/analysis/ai-screen`, default weights, 1 LLM call), **AI News' portfolio overview** (`/api/analysis/news-sentiment/portfolio`, up to **10** LLM calls — see above), **AI Prediction's portfolio overview** (`/api/analysis/prediction/portfolio`, pure math, 0 LLM calls), and **Swing** (`runSwingScreen()`, all 3 host contexts: VN30/VN100/Portfolio, no LLM). Portfolio + Sector still render server-side on first paint — nothing to prefetch. **Deliberate cost trade-off**: every `/analysis` visit now pays for up to 12 LLM calls total across the three LLM-backed surfaces, even if none of those tabs are opened — see `.cursor/rules/analysis-page-prefetch.mdc`.

### AI Prediction tab (Aug 2026) — Level 4, probabilistic return estimate, no trained ML model

Implements "AI Prediction — Level 4 (Probability, Not Certainty)". This app has no Python training pipeline, feature store, or multi-year price history to train/serve XGBoost/LightGBM/LSTM in-process, so this is a **statistical random-walk model**, not a real ML model — `model_used` says so explicitly rather than claiming a model family that isn't running.

- **`src/lib/analysis/prediction-model.ts`** (`buildPricePrediction(symbol, {horizonDays, combinedScore?})`) — daily log-returns over the trailing ~260 days give a drift/volatility estimate, scaled to the horizon under a random-walk assumption (`prob_price_up` = normal CDF of drift÷volatility at that horizon). The existing Technical+Fundamental **Combined score (Level 2, reused via `analyzeCombinedRow` — never recomputed)** nudges the drift by up to ±8 annualized percentage points; a neutral score of 50 applies zero tilt. Sentiment (Level 3) tilt is **not wired in** — would require triggering the LLM-backed News Sentiment pipeline per prediction, deliberately avoided to stay LLM-free. Output is percentages/probabilities only — **no absolute price target is ever computed**.
- **Backtest, always attached, never a bare prediction**: walk-forward only (each window uses only returns from before its own prediction point). Hit-rate/calibration use overlapping daily windows for sample size; Sharpe ratio/max drawdown use a separate non-overlapping sequence so the equity curve is valid (overlapping trades can't be chained without double-counting the same days). Predictions with under 40 days of history are marked `insufficient_data` rather than estimated from too little data.
- **Route**: `POST /api/analysis/prediction` (session auth) — `{symbol, horizonDays?}` → `PricePrediction`. `GET /api/analysis/prediction/portfolio` (`?horizonDays=`) → runs the same pipeline for **every** holding (`prediction-portfolio.ts`) — unlike AI News's LLM-backed overview, this is pure math, so it isn't capped at the top 10 by weight; `MAX_PREDICTION_HOLDINGS` (50) is a serverless-timeout safety valve only.
- **UI**: `PredictionPanel` (`src/components/analysis/prediction-panel.tsx`) — same ticker-lookup + portfolio-overview layout as `NewsSentimentPanel`, plus a 7/14/30/60/90-day horizon selector. Every result shows probability, expected return + 90% CI, and annualized volatility together (never one number in isolation), with the backtest and disclaimer always visible, not hidden behind a click. Cached `vnstocks:prediction-{SYM}-{horizon}` and `vnstocks:prediction-portfolio` (1h TTL — statistical, not LLM-backed, so no need for AI News's 2h cadence).
...
### Outcome tracking (Sep 2026) — the feedback loop, on the Scoring Rules tab

Until this landed, **nothing in the app measured whether any signal worked.** The `recommendation` table has always had `price_at_{30,90,365}d` / `return_pct_*` / `hit_*` columns, but nothing in this repo ever wrote them, and the /analysis Combined signals were never persisted at all — so every weight (0.60/0.40 Combined, AI Analyst's 26/24/18/14/10/8, AI Screening's six) was an unvalidated guess.

- **`src/lib/db/recommendation-outcomes.ts`** — three parts in order: `recordSignals` (persist today's signals; upserts on the existing `uq_recommendation_unique`, so re-running same-day refreshes rather than duplicating) → `evaluateDueOutcomes` (backfill a horizon once it matures) → `getSignalAccuracy` (realized hit-rate per label).
- **Price units**: both the entry and exit price come from the same `price_daily.close_px` column, so `returnPct` is a ratio of two like-scaled numbers and is **unit-independent by construction**. Do NOT route either side through `toVndPrice`/`priceKToVnd` — that reintroduces the thousands-vs-VND trap this deliberately sidesteps.
- **`hit` semantics**: bullish label → return > 0; bearish → return < 0; **HOLD/WATCH → `null`**, not false. They express "no action", so they have no direction to be right about; they're excluded from hit-rate denominators but their return is still recorded. `signalDirection()` is unit-tested precisely because a misclassification here silently inverts the track record instead of failing loudly.
- **Never fabricates**: a row whose entry price can't be resolved is skipped (a zero entry price would produce infinite return); a horizon with no trading session inside `SESSION_TOLERANCE_DAYS` (10, to clear Tet) stays unevaluated so a later run can pick it up once `price_daily` backfills.
- **Route** `/api/analysis/outcomes` — **GET is overloaded on purpose**: GET + `Bearer $CRON_SECRET` runs the loop, GET + user session reads the report, POST + either runs the loop. Vercel crons invoke paths with **GET**, so moving the cron path to POST-only would silently stop the schedule. (`/api/data/sync` has that exact bug today: its GET requires a session, so its configured cron gets a 401.) Cron: `30 9 * * 1-5`.
- **UI**: `TrackRecordPanel` at the top of the Scoring Rules tab — the rules below it say what the app *claims*; this says whether the claim held. A null hit-rate renders "—", **never 0%** ("not measured yet" ≠ "never right"), and sample size sits beside every percentage.
- `NO DATA` rows are filtered out before recording — they can never resolve to a hit and would dilute every denominator.

### Data coverage (Sep 2026) — `NO DATA` instead of a confident 50

A stock with no `technical_snapshot` used to score a flat technical **50** (`calculateTechnicalScore(null, …)`), and because *every* context check in `getRecommendationFromScore` (nearSupport, supportBroken, nearResistance, belowMa50, rsiOverbought) silently defaults to false without a snapshot, it fell through the score bands to a confident-looking **HOLD** — visually identical to a real one. The fundamental side compounded it: the no-snapshot fallback hardcoded six of ten inputs to null, capping the reachable score near **52** however good the company was.

- **`DataCoverage`** (`stock-analysis.ts`) — `{hasTechnical, fundamentalFields, totalFundamentalFields, level}` where `level` is `full | partial | none`. Carried on both `TechnicalAnalysisRow` and `CombinedAnalysisRow`.
- **No technical snapshot → `level: "none"` → recommendation is `NO_DATA_SIGNAL`** (`"NO DATA"`, exported from `technical-scoring.ts`). Technical is 60% of Combined *and* gates every context check, so its absence alone is disqualifying.
- **`CoverageMark`** renders `no data` / `thin` beside the symbol in the Combined and Technical tables. The ETF view already did this via `hasData`; the main tables never got it.
- Downstream consumers degrade correctly and were verified: `combinedStance()` (ai-holdings-panel) and the `includes("SELL")/includes("AVOID")` check (exit-strategy-panel) both treat `"NO DATA"` as neutral.
- `stockToFundamentals` now derives `profitGrowth` (and `revenueGrowth` as a fallback) from `stock.financials`' multi-year series, which was present all along and simply never read. **Still genuinely unavailable**: ROA, margins, debt/equity — all need balance-sheet totals (assets, equity) that `Financials` doesn't carry; `totalDebt` alone cannot produce a D/E ratio.

### Snapshot mapper width (Sep 2026)

`fundamental_snapshot` / `technical_snapshot` store far more than the app read. The Neon query has **no explicit `select`** (it already fetched every column) — the narrowing was purely in `FundRow`/`TechRow` and the mappers, so widening was type + mapper work only. `scripts/sync-neon-cache.ts`'s SQL needed widening too.

Now read: `epsGrowth3y`, `roic`, `currentRatio`, `pCashFlowRatio`, `adx14`, `atr14`.

**The rule for new inputs**: they are *fallbacks* for a missing primary, never overrides — so they can only lift a category that would otherwise score zero, and no stock with complete data changes score. `roic` backs ROE in quality; `currentRatio` backs debt/equity in stability (scored more conservatively, capped at 8 — liquidity speaks to short-term solvency, not capital structure).

**One deliberate exception**: `epsGrowth3y` is *preferred* over `epsGrowth` when present, because a 3-year series is genuinely better than one year and the YoY value was only ever a documented stand-in. This one **can** move a fully-covered stock's score — an intended correction, not a silent regression.

Still unread (available if needed): `quickRatio`, `cashRatio`, `evEbitda`, `psRatio`, `assetTurnover`, `inventoryTurnover`, `receivablesTurnover`, `bvps`, `sma10`, `ema12`, `ema26`, `vwap`, `macdHistogram`. **Verify a column is actually populated before scoring on it** — the app never writes these tables (an external stock-service does):
```
SELECT count(*) total, count(eps_growth_3y) eps3y, count(roic) roic, count(p_cash_flow_ratio) pcf FROM fundamental_snapshot
```
...
- **8 scored criteria**: `aboveMA20`, `aboveMA50`, `rsiStrong` (RSI > 60), `volumeSpike` (ratio ≥ 2×), `near52wHigh` (within 15%), `outperformsMarket` (stock % > VN-Index %), `leadingSector`, `positiveMomentum` (price > 0). Thresholds live in `swing-screener.ts` (`rsiStrong`, `near52wHigh`) and `getTechnicalSignals()` in `market-service.ts` (the MA20/MA50/Volume Ratio `signal` fields the screener reads) — the UI tooltips in `analysis-view.tsx` must be updated alongside either
- **MA20 ≠ MA50 (fixed Sep 2026)**: `getTechnicalSignals()` computed the `"MA 20"` entry's `signal` from `price > ma50` (copy-paste from the `"MA 50"` entry above it) while showing the real `ma20` as its `value`. `aboveMA20` and `aboveMA50` were therefore always the identical boolean — 8 criteria were really 7, and every stock scored both or neither, skewing the ≥6/8 ENTRY threshold both ways. `ma20` is now kept unrounded for the comparison and `Math.round`ed only for display (prices are in thousands of VND, so rounding before comparing loses real precision). Also affects the MA20 signal shown on `/stocks/[symbol]`
...
| `specialized.ts` | 6 pure agents: `financialAgent`, `valuationAgent` (returns `{report, detail}`), `technicalAgent`, `newsAgent` (reuses `classifyNewsItemByRules` from `news-classification.ts` — **not** its own word lists, and **not** the LLM path), `riskAgent` (safety score = 100−risk), `macroAgent` |
