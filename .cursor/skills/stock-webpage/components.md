...
| `/analysis` | `analysis/page.tsx` | Server | Portfolio / AI Analyst / AI Screening / **AI News** / **AI Prediction** / Sector / ETF / VN30 / VN100 / Avg Down / Exit Strategy / Scoring rules / **Principles**; Portfolio/VN30/VN100 have Fundamental/Technical/Combined/**Swing** sub-tabs | `combined-analysis`, `sector-analysis`, `etf-analysis`, shared portfolio cache |
...
| `sortable-column.tsx` | Client | **NEW (Aug 2026)** — `SortableColumn`, the flex-row equivalent of `SortableTableHeader` (which renders a `<th>` and only works inside `<table>`). Same `useTableSort`/`compareNumbers`/`compareStrings` pattern; used to add column sorting to the AI modules' card/row lists (AiHoldingsPanel, AiScreeningPanel, NewsSentimentPanel, PredictionPanel), which render div-based rows, not tables |
...
| File | Type | Purpose |
|------|------|---------|
| `analysis-view.tsx` | Client | Tabs: Portfolio / AI Analyst / **AI Screening** / **AI News** / **AI Prediction** / Sector / ETF / VN30 / VN100 / Avg Down / **Exit Strategy** / Scoring rules / Principles. Portfolio/VN30/VN100 sub-tabs: Fundamental / Technical / Combined / **Swing** (Aug 2026 — former standalone "Short Swing" tab, folded in since 4 of its 8 criteria overlapped the Technical sub-tab's signals; passes universe-appropriate `defaultSymbols` + `universeLabel` to `ShortSwingPanel` — VN30 symbols / top-30 VN100-by-score / your holdings). `TechnicalTable` has **current price (₫)** and **Volume Ratio** columns. **Lazy loading (extended Aug 2026)**: every lazy tab/sub-tab loads on open AND is **background-prefetched** once after first paint (`requestIdleCallback`) — VN30/VN100/ETF via `loadLazyUniverse`, plus **AI Analyst**, **AI Screening**, **AI Prediction**, and **Swing** (all 3 host contexts) via `prefetchAiHoldings`/`prefetchAiScreening`/`prefetchPrediction`/`prefetchSwing`, so tab switches are instant everywhere, not just the index universes. Prefetching the LLM-backed tabs is a deliberate cost trade-off — see `.cursor/rules/analysis-page-prefetch.mdc`. `loadingUniverses: Set<LazyUniverse>` drives per-tab spinners; a subtle "Preparing … in background" line shows while prefetch runs |
| `ai-holdings-panel.tsx` | Client | **NEW (Jul 2026)** — `/analysis` **AI Analyst** tab. Auto-runs `GET /api/analyst/portfolio` on open (cached `vnstocks:ai-holdings` 30 min; Re-run button). Renders value-weighted health score, verdict chips, portfolio summary, actions/risks, and a per-holding table with expandable 6-agent breakdown. **Timing reconciliation (Jul 2026)**: rows with a bullish verdict but `timingConfirmed:false` show action `WAIT` (not `ACCUMULATE`) + an amber "timing not confirmed" note in the expanded panel. **Cross-tab divergence (Aug 2026)**: takes a `combinedRows` prop (`portfolio.combined`, already loaded by `AnalysisView`) and flags rows whose verdict lands on the opposite bullish/bearish side from Portfolio → Combined's `recommendation` — icon in the row header + explanatory note in the expanded panel, not a weight change |
...
| `news-sentiment-panel.tsx` | Client | **NEW (Aug 2026)** — `/analysis` **AI News** tab. **Portfolio Overview** (top, auto-runs, background-prefetched): `GET /api/analysis/news-sentiment/portfolio` → expandable per-holding rows (top 10 by weight, full LLM classification each — see cost note in `news-sentiment-portfolio.ts`), cached `vnstocks:news-sentiment-portfolio` (2h); "Re-run" button. **Look up any ticker** (below, on-demand only — no sensible default ticker to prefetch): ticker input (same pattern as `StockEvaluationPanel`) → `POST /api/analysis/news-sentiment` → conflicts banner, news list (trust/category/sentiment/horizon/confidence badges), social sentiment card (bullish/bearish % always paired with sample size, buzz change reported neutrally). Both share a `NewsSentimentDetail` renderer. Last ad-hoc ticker+result persisted `vnstocks:news-sentiment-state`; per-ticker report cached `vnstocks:news-sentiment-{SYM}` (2h) |
| `prediction-panel.tsx` | Client | **NEW (Aug 2026)** — `/analysis` **AI Prediction** tab (Level 4, "Probability, Not Certainty"). Same layout as `NewsSentimentPanel` (Portfolio Overview auto-run + ad-hoc ticker lookup, shared `PredictionDetail` renderer) plus a 7/14/30/60/90-day horizon selector. **Portfolio Overview** (`GET /api/analysis/prediction/portfolio`) runs for **every** holding, not capped at 10 — pure math, no LLM call. Each result shows probability of positive return, expected return + 90% CI, and annualized volatility together (never one number alone), plus an always-visible backtest card (hit rate, calibration, Sharpe, max drawdown, sample sizes) and disclaimer — no prediction ships without its track record. Cached `vnstocks:prediction-{SYM}-{horizon}` / `vnstocks:prediction-portfolio` (1h) |
...
| `track-record-panel.tsx` | Client | **NEW (Sep 2026)** — realized hit-rate per Combined signal, rendered at the top of the **Scoring Rules** tab (rules below say what the app claims; this says whether it held). `GET /api/analysis/outcomes`. Display contract: a null hit-rate shows `—`, **never 0%** — "not measured yet" and "never right" are different claims; sample size (`n=`) sits beside every percentage so a 100% on n=2 reads as noise; HOLD/WATCH show "not directional" rather than a blank cell, so the exclusion reads as a decision, not missing data |
| `CoverageMark` (in `analysis-view.tsx`) | Client | **NEW (Sep 2026)** — `no data` / `thin` chip beside the symbol in the Combined + Technical tables, driven by `DataCoverage.level`. Without it a snapshot-less stock renders a flat technical 50 and a ~52-capped fundamental, landing mid-table and indistinguishable from a genuinely average stock. The ETF view already had this via `hasData` |
| `ShortSwingPanel` (in `analysis-view.tsx`) | Client | **Updated (Aug 2026)** — now rendered as the **Swing** sub-tab under Portfolio/VN30/VN100 (was its own top-level "Short Swing" tab). Takes `defaultSymbols` + `universeLabel` props ("VN30"/"VN100"/"Portfolio") so the "Load {label}" button and chip overflow text aren't hardcoded to VN30. Auto-runs on sub-tab open (table pre-populated); **input starts empty** for custom tickers; **chips** (click to append); scores 8 criteria: `aboveMA20`, `aboveMA50`, `rsiStrong`, `volumeSpike`, `near52wHigh`, `outperformsMarket`, `leadingSector`, `positiveMomentum`; ENTRY (≥6) / WATCH (3–5) / SKIP (<3); collapsible 10-step guide |
...
| `vnstocks:news-sentiment-{SYM}` | 2h | `NewsSentimentPanel` (`/analysis` AI News tab) — last `NewsSentimentReport` per ticker |
| `vnstocks:news-sentiment-portfolio` | 2h | `NewsSentimentPanel` — last `PortfolioNewsOverview` (top 10 holdings by weight) |
| `vnstocks:prediction-{SYM}-{horizon}` | 1h | `PredictionPanel` (`/analysis` AI Prediction tab) — last `PricePrediction` per ticker+horizon |
| `vnstocks:prediction-portfolio` | 1h | `PredictionPanel` — last `PortfolioPredictionOverview` (all holdings — no LLM call to cap) |
| `vnstocks:swing-{vn30\|vn100\|portfolio}` | 30 min | Swing sub-tab (`LOCAL_CACHE_KEYS.swingScreen(universeLabel)`) — last `SwingScreenResult` per host context, written by background prefetch or by `ShortSwingPanel` itself on a cache miss |
...
| `providers/stocktwits.ts` | Public Stocktwits symbol stream (`fetchStocktwitsMessages`) — free, no key; returns `null` (not `[]`) on any failure so callers never treat "unreachable" as "zero posts" |
...
| `analysis/scoring-rules.ts` | Scoring rules tab copy (includes `AI_SCREENING_RULES`, `AI_NEWS_SENTIMENT_RULES`) |
| `analysis/news-classification.ts` | Per-article LLM classification (`classifyNewsItems`) + rule-based fallback; `inferTrustTier()`. headline/source/timestamp/link always copied from the real fetched article, never LLM-generated |
| `analysis/social-sentiment.ts` | Stocktwits aggregation (`fetchSocialSentiment`) — bot/spam filtering, buzz-vs-retrievable-history estimate, always pairs % with sample size |
| `analysis/news-sentiment.ts` | Aggregates news + social into `NewsSentimentReport` (`buildNewsSentimentReport`); `conflicts[]` detection, disclaimer attached to every report; exports `newsStance`/`socialStance` for server-side row labeling |
| `analysis/news-sentiment-portfolio.ts` | Portfolio-wide overview (`buildPortfolioNewsOverview`) — full LLM pipeline per holding, capped at `MAX_OVERVIEW_HOLDINGS` (10) by weight; skipped holdings reported, not dropped |
| `analysis/prediction-config.ts` | Prediction horizon options/default + disclaimer — client-safe, zero imports |
| `analysis/prediction-model.ts` | Level 4 statistical prediction (`buildPricePrediction`) — random-walk drift/volatility from price history, tilted by the Level 2 Combined score (`analyzeCombinedRow`, never recomputed), plus walk-forward backtest (hit rate, calibration, Sharpe, max drawdown). No trained ML model, no LLM call |
| `analysis/prediction-portfolio.ts` | Portfolio-wide overview (`buildPortfolioPredictionOverview`) — runs every holding (pure math, no LLM cap needed), `MAX_PREDICTION_HOLDINGS` (50) is a serverless-timeout safety valve only |
| `analysis/swing-screener.ts` | Swing sub-tab's fetch+scoring logic (`runSwingScreen`) — extracted so both `ShortSwingPanel` and `AnalysisView`'s background prefetch call the same function, client-only |
...
| `/api/analysis/news-sentiment` | POST | **AI News Reading & Sentiment Analysis**: `{symbol}` → news classification + Stocktwits social sentiment → `NewsSentimentReport` (`buildNewsSentimentReport`). Powers `/analysis` AI News tab's ad-hoc ticker lookup | Session |
| `/api/analysis/news-sentiment/portfolio` | GET | Runs the full news-sentiment pipeline for your top 10 holdings by weight (`buildPortfolioNewsOverview`) → `PortfolioNewsOverview`. Powers `/analysis` AI News tab's Portfolio Overview; background-prefetched | Session |
| `/api/analysis/prediction` | POST | **AI Prediction — Level 4**: `{symbol, horizonDays?}` → statistical probability/return estimate (`buildPricePrediction`) → `PricePrediction`. Powers `/analysis` AI Prediction tab's ad-hoc ticker lookup | Session |
| `/api/analysis/prediction/portfolio` | GET | `?horizonDays=` — runs the same pipeline for every holding (`buildPortfolioPredictionOverview`) → `PortfolioPredictionOverview`. Powers `/analysis` AI Prediction tab's Portfolio Overview; background-prefetched | Session |
